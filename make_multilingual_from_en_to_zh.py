...
Usage:
python make_multilingual_from_en_to_zh.py

OR

python make_multilingual_from_en_to_zh.py \
    --Tmodel all-mpnet-base-v2 \
    --Smodel xlm-roberta-base \
    --max_seq_length 256 \
    --batch_size 32 \
    --epoch 4 \
    --model_save_path 
...
parser.add_argument('--Tmodel', dest='Tmodel', type=str, help='Monolingual teacher model', default='all-mpnet-base-v2')
parser.add_argument('--Smodel', dest='Smodel', type=str, help='Multilingual student model', default='xlm-roberta-base')
...
teacher_model_name = args.Tmodel  #Our monolingual teacher model, we want to convert to multiple languages
student_model_name = args.Smodel      #Multilingual base model we use to imitate the teacher model

max_seq_length = args.max_seq_length            #Student model max. lengths for inputs (number of word pieces)
train_batch_size = args.batch_size             #Batch size for training
inference_batch_size = 64           #Batch size at inference
max_sentences_per_language = 500000 #Maximum number of  parallel sentences for training
train_max_sentence_length = 250     #Maximum length (characters) for parallel training sentences

num_epochs = args.epoch                       #Train for x epochs
...
target_languages = set(['zh-cn', 'zh-tw'])    # We want to extend the model to these new languages. For language codes, see the header of the train file


output_path = args.model_save_path if args.model_save_path else "output/make-multilingual-"+"-".join(sorted(list(source_languages))+sorted(list(target_languages)))+"-"+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
